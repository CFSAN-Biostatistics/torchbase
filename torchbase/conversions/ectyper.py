...
def convert_local(
    ...
    _log.info("Starting ectyper conversion → %s/%s %s", namespace, name, version)
    ...
    if db_fasta is not None:
        _log.info("  splitting combined FASTA into O/H/other antigen files")
        locus_names = _split_combined_fasta(db_fasta, resources_dir)
        _log.debug("  split result: %s", locus_names)
    else:
        locus_names = []
